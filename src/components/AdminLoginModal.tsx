...
import { ShieldCheck, ShieldAlert, X, Lock, Eye, EyeOff, AlertTriangle, Clock, MessageSquare, Sparkles, RotateCcw } from 'lucide-react';
...
export const AdminLoginModal: React.FC<AdminLoginModalProps> = ({
  ...
  // Ensure fields always start completely blank and wipe any auto-injected browser credentials
  useEffect(() => {
    setEmail('');
    setPassword('');
  ...
  const handleLoadDemo = () => {
    setEmail('serviciosyaparaguay@gmail.com');
    setPassword('Servi270985#');
    setError(null);
    setDemoLoaded(true);
    setTimeout(() => setDemoLoaded(false), 3500);
  };
        ...
        {isBlocked ? (
          ...
          <>
            ...
            <form onSubmit={handleSubmit} autoComplete="off" className="space-y-4">
              {/* Inputs ocultos ficticios para desviar el autocompletado agresivo de contraseñas guardadas del navegador */}
              <input type="text" name="fake_admin_username_prevent" className="sr-only hidden" tabIndex={-1} autoComplete="off" aria-hidden="true" />
              ...
              {/* Quick Actions: Cargar Demo / Dejar en Blanco */}
              <div className="flex items-center justify-between pt-0.5 text-xs">
                <button
                  type="button"
                  onClick={handleLoadDemo}
                  className="inline-flex items-center gap-1.5 text-xs text-indigo-600 hover:text-indigo-800 font-semibold transition-colors cursor-pointer"
                >
                  <Sparkles className="w-3.5 h-3.5 text-indigo-500" />
                  <span>{demoLoaded ? '✓ Credenciales demo cargadas' : 'Cargar datos demo'}</span>
                </button>
